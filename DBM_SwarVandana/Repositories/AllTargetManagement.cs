...
namespace Repositories
{
    public class AllTargetManagement
    {
        ...
        public DataSet GetTargetDetais(int TargetType, int Month, string financialYear, int centerId)
        {
            string Query = "";
            if (TargetType == 3)
                Query = "select sr.EnrollmentNo,s.Name,c.Amount from [dbo].[StudentRenewal] sr,[dbo].[CenterTarget] c,[dbo].[Student] s where sr.RenewalId = c.EnqId and sr.StudentId= s.StudentId and  sr.RenewalId in (select EnqID from  [dbo].[CenterTarget] where  Type=" + TargetType + " and Month =" + Month + " and FinancialYear='" + financialYear + "' and CenterId='" + centerId + "')";
